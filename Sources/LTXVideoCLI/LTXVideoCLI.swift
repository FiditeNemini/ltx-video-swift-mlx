...
struct LipDub: AsyncParsableCommand {
    ...
    @Option(name: .long, help: "Segment chaining (image mode): short clip whose FIRST 9 frames are the previous segment's tail. Must be re-encoded with frame 0 at t=0 — an input seek (-sseof) yields a clip the extractor rejects; use: ffmpeg -i seg.mp4 -vf \"select='gte(n,N-9)',setpts=N/24/TB\" -r 24 -c:v libx264 -g 1 -crf 12 -pix_fmt yuv420p -an tail.mp4. Replaces the still image as the frame-0 anchor, preserving position AND motion across the cut. Requires --target-audio; trim 1 frame at concatenation. Mutually exclusive with --reference-video.")
