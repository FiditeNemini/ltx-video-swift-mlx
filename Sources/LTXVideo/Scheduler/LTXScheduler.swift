...
class LTXScheduler: @unchecked Sendable {
    ...
    func setTimesteps(
        ...
        if distilled {
            // Use predefined distilled sigma values directly (no dynamic shifting)
            // Matches Lightricks reference: raw DISTILLED_SIGMA_VALUES without transformation
            self.sigmas = DISTILLED_SIGMA_VALUES
            LTXDebug.log("Scheduler set: distilled mode (raw sigmas, \(DISTILLED_SIGMA_VALUES.count - 1) steps)")
        } else {
