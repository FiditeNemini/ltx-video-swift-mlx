...
public struct ChromeTraceExporter {
    ...
    public static func export(session: ProfilingSession) -> Data {
        ...
        traceEvents.append(metadataEvent(name: "process_name", pid: pid, tid: 0,
        ...
        traceEvents.append([
            ...
            "args": [
                "device": session.deviceArchitecture, "ram_gb": session.systemRAMGB,
                "model": session.modelVariant, "quantization": session.quantization,
                "resolution": session.resolution, "frames": session.frames,
                "steps": session.steps, "session_id": session.sessionId,
            ] as [String: Any],
        ])
