...
public struct BenchmarkResult: Sendable {
    ...
    public func generateReport() -> String {
        ...
        report += "  [\(formatDuration(totalStats.minMs / 1000)) - \(formatDuration(totalStats.maxMs / 1000))]\n"
    ...
    public static func aggregate(sessions: [ProfilingSession], warmupCount: Int) -> BenchmarkResult {
        guard let first = sessions.first else {
            return BenchmarkResult(
                ...
                peakMLXActiveMB: 0, peakProcessMB: 0, warmupRuns: warmupCount, measuredRuns: 0,
        ...
        // Sort by category order (deterministic)
        ...
        let stepStats = allStepDurations.isEmpty ? nil :
            computeStats(name: "Denoising Step", category: .denoisingStep, values: allStepDurations)
        ...
        return BenchmarkResult(
            ...
            warmupRuns: warmupCount, measuredRuns: sessions.count,
            modelVariant: first.modelVariant, quantization: first.quantization,
            resolution: first.resolution, frames: first.frames, steps: first.steps)
    }
