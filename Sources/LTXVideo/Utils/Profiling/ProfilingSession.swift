...
public final class ProfilingSession: @unchecked Sendable {
    ...
    /// Metadata — set before recording begins, read after recording completes.
    ...
    public func generateReport() -> String {
        ...
        // Sort phases by category order (deterministic)
        ...
        if !stepDurations.isEmpty {
            ...
            report += "  Average: \(formatDuration(avgMs / 1000))  Std: \(formatDuration(stdMs / 1000))\n"
            ...
            for count in [8, 15, 30, 40] {
                report += "    \(String(format: "%2d", count)) steps: \(formatDuration(avgMs * Double(count) / 1000))\n"
            }
        }
